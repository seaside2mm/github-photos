...
class MegaDepthDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image1, mask1, scale1 = read_megadepth_gray(
            ...
            # np.random.choice([self.augment_fn, None], p=[0.5, 0.5]))
        # depth_path0 = "../input/depth-masks-imc2022/depth_maps/" + img_name0.split("/")[-3] + '/' + img_name0.split("/")[-1]
        # depth_path1 = "../input/depth-masks-imc2022/depth_maps/" + img_name1.split("/")[-3] + '/' + img_name1.split("/")[-1]
        ...
        # del R1
        # del Tv1
        H_0to1 = self.H_matrix[idx].replace('{','').replace('}','').replace("'", "")
        a = H_0to1.split(" ")
        H_0to1 = np.asarray([float(x) for x in H_0to1.split(" ")]).reshape(3, 3)
        H_0to1 = torch.tensor(H_0to1, dtype=torch.float)
        H_1to0 = H_0to1.inverse()
        data = {
            'image0': image0,  # (1, h, w)
            'image1': image1,
            'H_0to1': H_0to1,  # (3, 3)
...
def train():
    ...
    print("config.TRAINER.TRUE_BATCH_SIZE:", config.TRAINER.TRUE_BATCH_SIZE, " ",  args.num_nodes)
    ...
    # lightning data
    data = pd.read_csv("../input/imc-gt/train-new.csv")
    data_module = MultiSceneDataModule(args, config, data[:1])
    gc.collect()
